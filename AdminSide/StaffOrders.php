...
if (isset($_POST['update_status'])) {
    ...
    if (in_array($status, $allowed_transitions[$current_status])) {
        ...
        header("Location: StaffOrders.php?success=updated");
        exit();
    } else {
        header("Location: StaffOrders.php?error=invalid");
        exit();
    }
}
...
<title>Staff Orders | Device Market</title>
...
<aside class="w-64 bg-[rgb(116,142,159)] text-white flex flex-col shadow-xl">
    ...
    <nav class="flex-1 p-4 space-y-2">
        <a href="StaffOrders.php" class="sidebar-link active flex items-center py-3 px-4 rounded-lg transition">
        ...
        <a href="StaffReports.php" class="sidebar-link flex items-center py-3 px-4 rounded-lg transition">
...
<form method="GET" class="flex flex-wrap items-center gap-4">
    <div class="flex flex-wrap gap-3">
        ...
        ?>
        <a href="StaffOrders.php<?= $key ? '?filter=' . $key : '' ?>" class="px-5 py-2.5 rounded-lg font-semibold transition <?= $bgClass ?>">
    ...
    <div class="flex items-center gap-3 ml-auto">
        ...
        <input type="date" name="from_date" value="<?= htmlspecialchars($from_date) ?>" class="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[rgb(116,142,159)]">
        <label class="font-semibold text-gray-700">To:</label>
        <input type="date" name="to_date" value="<?= htmlspecialchars($to_date) ?>" class="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[rgb(116,142,159)]">
        <button type="submit" class="bg-[rgb(116,142,159)] text-white px-4 py-2 rounded-lg font-semibold hover:bg-[rgb(100,123,136)] transition">
            <i class="fas fa-filter mr-2"></i>Apply
        </button>
        <a href="StaffOrders.php" class="bg-gray-300 text-gray-800 px-4 py-2 rounded-lg font-semibold hover:bg-gray-400 transition">
...
<td class="px-6 py-4">
<span class="px-3 py-1 text-xs rounded-full font-semibold <?= $order['status'] == 'completed' ? 'bg-green-100 text-green-700' : ($order['status'] == 'pending' ? 'bg-yellow-100 text-yellow-700' : ($order['status'] == 'processing' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700')) ?>">
...
<select name="status" class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[rgb(116,142,159)]" <?= ($order['status'] == 'completed' || $order['status'] == 'cancelled') ? 'disabled' : '' ?>>
...
<button type="submit" name="update_status" class="bg-[rgb(116,142,159)] hover:bg-[rgb(100,123,136)] text-white px-4 py-2 rounded-lg text-sm transition font-semibold" <?= ($order['status'] == 'completed' || $order['status'] == 'cancelled') ? 'disabled' : '' ?>>
